...
- **G1. Project home screen.** Done — CapCut-style gallery listing all projects from `default_projects_dir()`, with name, thumbnail, duration, last-modified; card actions: open, rename, delete. New Tauri commands: `list_projects`, `rename_project`, `delete_project`, `project_thumbnail`. Naming: quick-start/new-project default to the current local date (`Jul 18, 2026`, deduped with a `(2)`, `(3)`, … suffix — see `next_free_project_name` in `src-tauri/src/lib.rs`). Thumbnails: `project_thumbnail` regenerates when missing *or* stale (project file mtime newer than the cached thumb), sampling a representative frame (`renderly_core::perceive::representative_frame_time`) instead of always `t=0`, so fade-ins/black leaders don't dominate gallery cards. Remaining: duplicate action.
